<?php

/*
+--------------------------------------------------------------------------
|   Invision Power Board v2.3.4 Chinese Language Package
|   =======================================================================
|   作者: Skylook
|   =======================================================================
|   网站: http://www.ipbchina.com
|   发布: 2008-01-25
|   语言包版权归 IPBChina.COM 所有
+--------------------------------------------------------------------------
*/

$lang = array (

// 2.1

'cs_posts' => "帖子",
'cs_members' => "会员",
'cs_active'  => "在线会员",
'q_topics'   => "列表中的主题:",
'q_posts'    => "帖子:",

// added in 2.0

'enter_search_words' => "请输入搜索关键词...",
'sub_forum_title' => "论坛",

//

'whoschatting_total' => "位会员在线聊天",
'whoschatting_loadchat' => "在线聊天",
'whoschatting_none'  => "此刻没有会员在线聊天",
'whoschatting_delay' => "每 %s 分钟更新一次",

'rd_hits'       => "转向次数",
'oul_show_more' => "分类显示详细资料:",
'oul_click' => "最后点击",
'oul_name'  => "会员名称",

'qli_title'				=> "快速登录",
'qli_name'				=> "会员名称",
'qli_go'				=> "登录",

by					=>	"作者",

'calender_f_title'		=> "未来 %s 天内即将发生的日历事件",

'no_calendar_events'	=> "没有即将发生的日历事件",

'newslink'				=> "最新消息:",

'welcome_back_text'		=> "欢迎回来,您上一次访问论坛的时间是",

'sm_today_posters'		=> "今日发帖前20名",
'sm_all_posters'		=> "发帖总数前20名",
'sm_forum_leaders'		=> "论坛管理团队",
'sm_todays_posts'		=> "今日最活跃主题",

protected_forums		=>	"加密论坛",
no_birth_users			=>	"今天没有会员过生日",
total_forums			=>	"论坛总数:",
public_forums			=>	"开放论坛总数",
public_members			=>  "位注册会员",

'total_word_string'		=>  "帖子总数:<b><#posts#></b> 篇<br />注册会员:<b><#reg#></b> 人<br />最后注册:<b><a href='<#link#>'><#mem#></a></b>",


'birthday_header'		=> "今天过生日的会员",

'replies'					=>	"回复",
'board_stats'				=>	"论坛统计",

'guests'					=>	"位游客",
'posts'					=>	"主题数",
'total_of'				=>	"总计",
'browser_user_list'		=>	"查看详细列表",

'forum_leader'			=>	"版主:",
'birth_user'				=>	"位会员过生日",
'forum_users'				=>	"论坛在线人数",
'forums'					=>	"论坛",
'restricted_forums'		=>	"受限论坛",
'invite_submit'			=>	"提交",
'all_times'				=>	"所有时间",
'new_posts'				=>	"查看新帖",
'anon_members'			=>	"位隐身会员",
'd_post_read'				=>	"标记所有帖子为已读",
'last_post_info'			=>	"最后更新",
'birth_users'				=>	"位会员过生日",
'forum_off'				=>	"只读论坛",

cat_name				=>	"论坛",
in						=>	"主题",
registered_mems			=>	"注册会员",
topics					=>	"主题",
f_protected				=>	"加密论坛",
d_delete_cookies		=>	"删除论坛 Cookies 信息",
forum_icons				=>	"论坛图例",



info					=>	"信息",
active_users			=>	"位会员在最近 %s 分钟内在线",

f_none					=>	"----",
most_online				=>	"在线峰值:<b><#NUM#></b> 人,发生时间 <b><#DATE#></b>.",
no_new					=>	"没有新帖",

);
?>
