...
class sql_idm_queries extends db_driver_mysql
{
    ...
    {
	    return "SELECT COUNT(file_id) as files, SUM(file_downloads) as downloads, SUM(file_views) as views FROM ".SQL_PREFIX."downloads_files
    ...
    {
	    return "SELECT COUNT(*) as files FROM ".SQL_PREFIX."downloads_files
    ...
    function acp_get_stats( $a=array() )
    {
	    if( $a['groupby'] == 'dtime' )
	    {
		    return "SELECT d.did, {$a['type']}, d.{$a['groupby']} as indicator, FROM_UNIXTIME(d.dtime, '%M %Y') as dtime, f.file_name
	    				FROM ".SQL_PREFIX."downloads_downloads d
	    				LEFT JOIN ".SQL_PREFIX."downloads_files f ON(f.file_id=d.dfid)
	    				GROUP BY dtime ORDER BY num DESC LIMIT 0,{$a['limit']}";
    	}
    	else
    	{
	    	return "SELECT d.*, {$a['type']}, d.{$a['groupby']} as indicator, f.file_name
	    				FROM ".SQL_PREFIX."downloads_downloads d
	    				LEFT JOIN ".SQL_PREFIX."downloads_files f ON(f.file_id=d.dfid)
	    				GROUP BY d.{$a['groupby']} ORDER BY num DESC LIMIT 0,{$a['limit']}";
    	}
    }
    ...
    {
	    return "SELECT COUNT(file_id) as submissions, MAX(file_id) as last_id, file_submitter FROM ".SQL_PREFIX."downloads_files
	    		GROUP BY file_submitter LIMIT 10";
    }
    ...
    {
	    return "SELECT COUNT(did) as downloads, MAX(did) as the_id, dmid FROM ".SQL_PREFIX."downloads_downloads
	    		GROUP BY dmid LIMIT 10";
    }
